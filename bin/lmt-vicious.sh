...
vicious_start() {
    /bin/su - simon -c "/bin/echo 'vicious.activate()' | /usr/bin/awesome-client"
    /bin/echo "Activated Vicious widgets for Awesome window manager."
}

vicious_stop() {
    /bin/su - simon -c "/bin/echo 'vicious.suspend()' | /usr/bin/awesome-client"
    # Some widgets are important while running on battery, BAT widget in particular
    /bin/su - simon -c "/bin/echo 'vicious.activate(batwidget)' | /usr/bin/awesome-client"
