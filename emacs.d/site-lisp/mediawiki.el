...
;; Last Modified: <2010-06-27 01:12:25 mah>

(defconst mediawiki-version "2.1.3a"
      ...
      (while start
        ...
        (let ((el (match-string 1 form))
          ...
          (unless (string-match "type=[\"']submit['\"]" el)
            (let ((el-value (string-match "value=[\"']\\([^\"']*\\)['\"]" el)))
              ;; set the buffer-local form variables
              (if el-value
                  (add-to-list 'vars (cons el-name (match-string 1 el)))
                (add-to-list 'vars (cons el-name "")))))
...
get a cookie."
  ...
  (lexical-let ((user (or (mediawiki-site-username sitename)
                ...
                (sitename sitename))
    (url-http-get
     (mediawiki-make-url "Special:Userlogin" nil sitename)
     nil (get-buffer-create " *mediawiki-form*")
     (lambda (str args)
